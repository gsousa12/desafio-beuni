...
import { enqueueEmployeeBirthdayGift, checkForExistingGiftJobs } from "./queue";
import logger from "./logger";
import { BirthdayJobData } from "packages/types/dist";

const BATCH_SIZE = Number(process.env.BIRTHDAY_BATCH_SIZE ?? 100);
const BATCH_DELAY_MS = Number(process.env.BIRTHDAY_BATCH_DELAY_MS ?? 500);

export class BirthdayCheckService {
  static async executeBirthdayCheck(): Promise<void> {
